...
```
Here `model_dir` is the Qwen3-TTS MNN model directory, `text` is the input sentence, `max_frames` caps generated codec frames (about 80 ms per frame; use 64-128 for short text and raise it for longer text, EOS may stop earlier), `dump_dir` saves wav/bin debug outputs, `language` defaults to `auto` and supports `auto/chinese/english/german/italian/portuguese/spanish/japanese/korean/french/russian`, `--ref_audio <wav>` supplies a reference voice wav; Qwen3-TTS resamples it to 24 kHz with the soxr-like high-quality resampler on load. `auto` requires reference audio; without reference audio, specify the language explicitly. `--normalize [target_peak]` peak-normalizes only the saved wav with default target 1.
