...
```
其中 `model_dir` 是 Qwen3-TTS MNN 模型目录，`text` 是要合成的文本，`max_frames` 是最多生成的 codec 帧数（每帧约 80ms，短句建议 64-128，长句可提高，遇到 EOS 会提前停止），`dump_dir` 用于保存 wav/bin 调试输出，`language` 默认是 `auto`，可选 `auto/chinese/english/german/italian/portuguese/spanish/japanese/korean/french/russian`，`--ref_audio <wav>` 指定参考音色 wav（加载时会重采样到 24 kHz）。使用 `auto` 时必须提供参考音频；没有参考音频时需要显式指定语言。`--normalize [target_peak]` 只对保存的 wav 做峰值归一化，不填值时为 1。
